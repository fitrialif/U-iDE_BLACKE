...
dt = pandas.read_csv("mixed_models/data.csv", header=None)
dt.head()
dataset = dt.values
dataset = dataset[1:]
...
for epoch in range(1,1000):
    ...
    torch.save(model.state_dict(),'mixedL2.pt') # make .pt file have different names

## LOSS
plt.figure()
plt.title("loss over epochs")
plt.plot(overall_loss)
plt.savefig("mixedL2_loss.png")

## accuracies
plt.figure()
plt.title("accuracy over epochs")
plt.plot(accuracies)
plt.savefig("mixedL2_acc.png")
...
plt.savefig("mixedL2_conf.png")

with open("mixedL2_acc.blacke","w") as f:
